...
public class EventBus
{
    ...
    public static void PublishAnalyticsEvent<A>(A published_event) where A : AnalyticsEvent
    {
        // Publish original event as well
        Publish(published_event);

        /* Use type T to identify correct subscriber list (correct "topic") */
        Type t = typeof(AnalyticsEvent);

        if (_topics.ContainsKey(t))
        {
            IList subscriber_list = new List<Subscription<AnalyticsEvent>>(_topics[t].Cast<Subscription<AnalyticsEvent>>());

            /* This is a collection of subscriptions that have lost their target object. */
            List<Subscription<AnalyticsEvent>> orphaned_subscriptions = new List<Subscription<AnalyticsEvent>>();

            foreach (Subscription<AnalyticsEvent> s in subscriber_list)
            {
                if (s.callback.Target == null || s.callback.Target.Equals(null))
                {
                    /* This callback is hanging, as its target object was destroyed */
            ...
            /* Unsubcribe orphaned subs that have had their target objects destroyed */
            foreach (Subscription<AnalyticsEvent> orphan_subscription in orphaned_subscriptions)
            {
                EventBus.Unsubscribe<AnalyticsEvent>(orphan_subscription);
            }
